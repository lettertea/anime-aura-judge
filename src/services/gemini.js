...
function buildSystemPrompt(mediaType = 'ANIME') {
  const isManga = mediaType === 'MANGA'
  const medium = isManga ? 'manga' : 'anime'
  const mediumNoun = isManga ? 'manga' : 'anime'
  const verb = isManga ? 'read' : 'watch'
  const comfortNoun = isManga ? 'comfort reads' : 'comfort shows'
  return (
    `You are a perceptive ${medium} critic who reads people's taste from their 3x3 ${medium} grids. ` +
    ...
    (isManga ? ` These are printed/serialized manga — never reference animation, voice acting, episodes, soundtracks, dubs or subs.\n\n` : `\n\n`) +
    `Instructions:\n` +
    ...
    `3. modifiers: An array of exactly 9 entries (one per ${medium} title in the exact same order) evaluating each ` +
    ...
    `complementary genres interact (e.g., heavy drama balanced by ${comfortNoun}, psychological picks alongside ` +
    ...
    `throughline of their taste (what they seem to seek from ${mediumNoun}, how their picks complement or clash with each ` +
    `other as a collection), and close with a grounded, forward-looking note about what this says about the ${isManga ? 'reader' : 'viewer'} ` +
    ...
    `7. roasts: An array containing exactly 9 entries (one per ${medium} title in the exact same order): a witty, ` +
    `teasing one-liner about why they picked that ${isManga ? 'series' : 'show'} — the kind of joke that stings slightly because it's ` +
...
function buildUserPayload(scoreResult, selectedAnime, sheet) {
  ...
  const payloadKey = mediaType === 'MANGA' ? 'manga' : 'anime'
  return JSON.stringify({
    mediaType,
    [payloadKey]: selectedAnime.map((a, i) => ({
      slot: i + 1,
      title: a.title,
...
async function requestOnce(scoreResult, selectedAnime, sheet, mediaType) {
  ...
  const response = await client.chat.send({
    ...
    messages: [
      { role: 'system', content: buildSystemPrompt(mediaType) },
...
export async function getAuraVerdict(scoreResult, selectedAnime) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const text = await requestOnce(scoreResult, selectedAnime, sheet, mediaType)
